...
local function SrlzArgs(...) 
  local Srlzd = {}; -- For performance reasons

  for i,v in Pairs({...}) do
    local valueType = Type(v);
    local SrlzIndex = #Srlzd + 1;
    if valueType == "string" then
      Srlzd[SrlzIndex] = format("\27[32m\"%s\"\27[0m", v);
    elseif valueType == "table" then
      Srlzd[SrlzIndex] = Srlz(v, 0);
    else
      Srlzd[SrlzIndex] = Tostring(v);
    end;
  end;
...
Srlz = function(tbl, scope, checked) 
  checked = checked or {};
  ...
  -- Remove last comma
  local lastValue = Srlzd[#Srlzd];
  if lastValue then
    Srlzd[#Srlzd] = sub(lastValue, 0, -3) .. "\n";
  end;
...
function Srlzr.Srlz(tbl)
  if Type(tbl) ~= "table" then
    error("invalid argument #1 to 'Srlz' (table expected)");
  end;
  Assert(Type(tbl) == "table", "");
  return Srlz(tbl);
end;

function Srlzr.FormatArguments(...) 
  return SrlzArgs(...);
end;

function Srlzr.FormatString(str) 
...
function Srlzr.UpdateConfig(options) 
